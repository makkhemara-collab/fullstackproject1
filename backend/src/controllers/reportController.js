...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const lastMonthSales = lastMonthSalesQuery.reduce(
      (sum, sale) => sum + (Number(sale?.amount) || 0),
      0,
    );
...
const getTopSellingProducts = async (req, res) => {
  try {
    ...
    const topProducts = await OrderItem.findAll({
      ...
      group: [
        "prd_id",
        "tbl_master_product.prd_id",
        "tbl_master_product.prd_name",
        "tbl_master_product.photo",
        "tbl_master_product.unit_cost",
      ],
    ...
    res.json({
      ...
      data: topProducts.map((p) => ({
        ...
        totalRevenue: parseFloat(p.getDataValue("totalRevenue") || 0).toFixed(
          2,
        ),
      })),
    });
...
const getMonthlySales = async (req, res) => {
  try {
    ...
    const sales = await Sale.findAll({
      attributes: [
        [literal('EXTRACT(MONTH FROM "sale_date")'), "month"],
    ...
    sales.forEach((sale) => {
      ...
      if (monthIndex >= 0 && monthIndex < 12) {
        monthlyData[monthIndex].totalAmount = parseFloat(
          sale.getDataValue("totalAmount") || 0,
        );
        monthlyData[monthIndex].totalTransactions = parseInt(
          sale.getDataValue("totalTransactions") || 0,
        );
      }
    });
...
const getDailySales = async (req, res) => {
  try {
    ...
    const sales = await Sale.findAll({
      attributes: [
        [literal('EXTRACT(DAY FROM "sale_date")'), "day"],
        [fn("SUM", col("amount")), "totalAmount"],
        [fn("COUNT", col("sale_id")), "totalTransactions"],
      ],
      where: {
    ...
    sales.forEach((sale) => {
      ...
      if (dayIndex >= 0 && dayIndex < daysInMonth) {
        dailyData[dayIndex].totalAmount = parseFloat(
          sale.getDataValue("totalAmount") || 0,
        );
        dailyData[dayIndex].totalTransactions = parseInt(
          sale.getDataValue("totalTransactions") || 0,
        );
      }
    });
